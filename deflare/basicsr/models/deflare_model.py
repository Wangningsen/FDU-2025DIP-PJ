...
class DeflareModel(SRModel):
    ...
    def feed_data(self, data):
        ...
        depth_pro_depth = self.predict_depth_pro(img) 
        self.lq = torch.from_numpy(np.concatenate((data['lq'], depth_pro_depth.reshape((1, 1, 512, 512))), axis=1)).to(self.device)

    def predict_depth_pro(self, img_batch):
        batch_size, h, w, _ = img_batch.shape
        batch_depth = np.zeros((batch_size, h, w), dtype=np.float32)

        with torch.no_grad():
            for i in range(batch_size):
                current_img = Image.fromarray(img_batch[i])
                transformed_image = self.depth_transform(current_img)
                ...
                depth_resized = cv2.resize(depth_np, (w, h), interpolation=cv2.INTER_CUBIC)

                depth_clipped = np.clip(depth_resized, 0, 300)

                depth_max = depth_clipped.max()
                depth_min = depth_clipped.min()

                if depth_max - depth_min > np.finfo("float").eps:
                    normalized_depth = (depth_clipped - depth_min) / (depth_max - depth_min)
                else:
                    normalized_depth = np.zeros_like(depth_clipped)

                batch_depth[i] = normalized_depth
